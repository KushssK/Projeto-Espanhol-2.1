...
/**
 * Ordem pedagógica ideal para o módulo Conversação.
 *
 * Progressão:
 *  1. Saudações — o primeiro contato com o idioma
 *  2. Apresentação — como se apresentar (nome, profissão)
 *  3. Cotidiano — situações do dia a dia
 *  4. Perguntas e Respostas — praticar fluência com perguntas
 *  5. Compras — necessidade cotidiana (números, preços)
 *  6. Supermercado — compra específica (alimentos, pesos)
 *  7. Restaurante — interação em serviços de alimentação
 *  8. Ligação Telefônica — exigem boa escuta e fala
 *  9. Hotel — cenário de viagem
 * 10. Aeroporto — viagem mais complexa
 * 11. Reunião e Trabalho — contexto profissional/avançado
 */
...
export async function seedConversacaoLessons(): Promise<void> {
  try {
    ...
    // Buscar videoUrls existentes para detectar duplicatas e reordenação
    ...
    let reordered = 0;
    for (const video of CONVERSACAO_VIDEOS) {
      const existingLesson = existingLessons.find(l => l.videoUrl === video.url);
      if (existingLesson) {
        // Buscar a aula completa para verificar orderIndex
        const fullLesson = await prisma.lesson.findFirst({
          where: { videoUrl: video.url },
          select: { id: true, orderIndex: true },
        });
        if (fullLesson && fullLesson.orderIndex !== video.orderIndex) {
    ...
    if (created > 0 || reordered > 0) {
      ...
      console.log(`🎬 [SEED] Conversação: ${parts.join(', ')}.`);
    } else {
      console.log(`🎬 [SEED] Conversação: todas as ${CONVERSACAO_VIDEOS.length} videoaulas já existem e estão ordenadas.`);
    }
  } catch (err) {
